...
def generate_responses(model_pipeline, messages_prompt, secondary_message_prompt, INFERENCING_API, correlation_id):
    try:
        ...
        time.sleep(1)
        secondary_result = process_with_llm(model_pipeline, secondary_message_prompt, use_inference_api=INFERENCING_API)
...
def handle_detection(processed_result, secondary_result, modified_task_data, messages_prompt, 
    ...
    final_processed_data = process_with_llm(model_pipeline, updated_prompt, use_inference_api=INFERENCING_API)
...
# from utils.string_parser import parse_code_smell_output
# from utils.file_hasher import create_mapped_task_data, revert_task_data_keys
# from utils.prompt import create_prompt, update_prompt
# from llm_processor import process_with_llm
# from logger_config import get_logger
...
#         logger.info(f"Task type: {task_type}")
#         logger.info(f"Task job: {task_job}")
#         messages_prompt, secondary_message_pompt, processed_data = create_prompt(task_type, modified_task_data, task_job,knowledge_base_detection, nn_model)  #knowledge_base_detection, nn_model
#         try:
#             logger.info(f"Generating response for task message with correlation ID: {correlation_id}")
#             processed_result = process_with_llm(model_pipeline, messages_prompt, use_inference_api=INFERENCING_API)
#             secondary_result = process_with_llm(model_pipeline, secondary_message_pompt, use_inference_api=INFERENCING_API)
#             if processed_result == "None" and secondary_result == "None":
#                 return final_result, task_status
#             if task_type =="detection":
#                 processed_data = parse_code_smell_output(processed_result, modified_task_data)
#                 secondary_data = parse_code_smell_output(secondary_result, modified_task_data)
#                 updated_prompt = update_prompt(messages_prompt, processed_data, secondary_message_pompt, secondary_data, task_type)
                
#                 final_processed_result = process_with_llm(model_pipeline, updated_prompt, use_inference_api=INFERENCING_API)
#                 final_processed_data = parse_code_smell_output(final_processed_result, modified_task_data)
#                 processed_data.update(final_processed_data)
